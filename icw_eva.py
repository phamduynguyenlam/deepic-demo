...
def _normalize_epoch_rewards_by_problem(
    ...
    """Normalize rewards per benchmark problem within the current epoch.

    PPO still uses advantage normalization later, but this prevents one
    benchmark problem from dominating centralized training because its raw
    reward scale is larger. After reward replacement, PPO targets are
    recomputed per (problem, dim) trajectory.
    """
    if not trajectories:
        return {}
...
def train_icw_multisource_ppo(args, target_problem: str, self_train_only: bool = False) -> ICW:
    ...
    # The requested target is a 15-vCPU runtime; use 12 workers by default
    # to leave headroom for the Ray driver and BLAS/OpenMP overhead.
    ...
    for epoch in range(args.start_epoch, multisource.TRAIN_EPOCHS):
        ...
        if parallel_workers == 1:
            ...
            if ray is None:
                ...
                # Fallback when Ray is not installed: still parallelize rollouts.
                # Each worker is CPU-only and already sets torch threads=1.
                ...
                remote_collect = ray.remote(num_cpus=1)(_collect_one_icw_env_trajectory)
        ...
        if epoch_trajectories:
            ...
            # Use a larger minibatch only when the centralized batch is large enough.
            ppo_minibatch_size = 64 if len(epoch_trajectories) > 200 else 32
        ...
        epoch_mean_rewards.append(raw_epoch_mean)
        print(f"Epoch {epoch + 1} raw mean reward: {raw_epoch_mean:.6f}")
        print(f"Epoch {epoch + 1} normalized mean reward: {epoch_mean:.6f}")
        print(f"Epoch {epoch + 1} mean PPO total loss: {epoch_mean_loss:.6f}")

        # Select checkpoints by raw reward; normalized reward is near zero by construction.
        if raw_epoch_mean > best_epoch_mean_reward:
            best_epoch_mean_reward = raw_epoch_mean
