...
def load_hv_deepic2_class():
    ...
    spec = importlib.util.spec_from_file_location("deepic_agent_local_hv2_dtlz4", path)
...
def _epoch_checkpoint_path(epoch_number: int) -> Path:
    return Path(__file__).resolve().parent / f"hv_deepic2_dtlz4_epoch_{epoch_number}.pth"
...
def load_or_train_hv_deepic2(args):
    ...
    return train_hv_deepic2_dtlz4(args)


def run_hv_deepic2_dtlz4(args, deepic, plot: bool = True, initial_archive_x: np.ndarray = None):
...
def run_comparison(args):
    ...
    hv_deepic2_result = run_hv_deepic2_dtlz4(
        args,
        deepic=deepic,
        plot=False,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train HV_DeepIC2 on 15D/20D/25D mixed-source problems and evaluate on DTLZ4."
    )
