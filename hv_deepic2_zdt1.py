...
def load_hv_deepic2_class():
    ...
    spec = importlib.util.spec_from_file_location("deepic_agent_local_hv2", path)
...
def load_or_train_hv_deepic2(args):
    ...
    return train_hv_deepic2_zdt1(args)


def run_hv_deepic2_zdt1(args, deepic, plot: bool = True, initial_archive_x: np.ndarray = None):
    ...
    if plot:
        ...
        plt.title(f"{args.dim}D ZDT1 Hypervolume Progress (HV_DeepIC2)")
        ...
        plt.figure(figsize=(8, 5))
        plt.title(f"{args.dim}D ZDT1 Pareto Front (HV_DeepIC2)")
        plt.scatter(final_front[:, 0], final_front[:, 1], s=24, alpha=0.8, label="HV_DeepIC2")
...
def run_comparison(args):
    ...
    shared_init_x = hv_zdt1.latin_hypercube_sample(
        lower=0.0,
        upper=1.0,
        n_samples=args.archive_size,
        dim=args.dim,
    ...
    hv_deepic2_result = run_hv_deepic2_zdt1(
        args,
        deepic=deepic,
        plot=False,
    ...
    eic_args = hv_zdt1.build_args_namespace(args)
    eic_result = hv_zdt1.nsga_eic.run_nsga_eic_problem(
        eic_args,
    ...
    plt.scatter(
        hv_deepic2_result["final_front"][:, 0],
        hv_deepic2_result["final_front"][:, 1],
        s=24,
        alpha=0.8,
        label="HV_DeepIC2",
    )
    plt.scatter(eic_result["final_front"][:, 0], eic_result["final_front"][:, 1], s=24, alpha=0.8, label="NSGA-EIC")
    plt.plot(hv_deepic2_result["true_front"][:, 0], hv_deepic2_result["true_front"][:, 1], "k-", linewidth=2, label="True Pareto Front")
    plt.xlabel("f1")
    plt.ylabel("f2")
    plt.grid(True)
    plt.legend()
