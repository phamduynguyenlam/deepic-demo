...
def load_hv_deepic2_class():
    ...
    spec = importlib.util.spec_from_file_location("deepic_agent_local_hv2_dtlz7", path)
...
PROBLEM_NAME = "DTLZ7"
MODEL_PATH = "hv_deepic2_dtlz7.pth"
REWARD_LOG_DIR = Path(__file__).resolve().parent / "reward_logs"
REWARD_LOG_PATH = REWARD_LOG_DIR / "hv_deepic2_dtlz7_train_rewards.json"
...
def _epoch_checkpoint_path(epoch_number: int) -> Path:
    return Path(__file__).resolve().parent / f"hv_deepic2_dtlz7_epoch_{epoch_number}.pth"
...
def train_hv_deepic2_dtlz7(args):
    ...
    for epoch in range(args.start_epoch, 50):
        ...
        if (epoch + 1) % 5 == 0:
            multisource.save_colab_model_checkpoint(
                deepic.state_dict(),
                f"hv_deepic2_dtlz7_epoch_{epoch + 1}.pth",
            )
    ...
    _save_reward_log(
        REWARD_LOG_PATH,
        {
            "script": "hv_deepic2_dtlz7.py",
            "mode": "train_hv_deepic2_dtlz7",
...
def load_or_train_hv_deepic2(args):
    ...
    return train_hv_deepic2_dtlz7(args)


def run_hv_deepic2_dtlz7(args, deepic, plot: bool = True, initial_archive_x: np.ndarray = None):
...
def run_comparison(args):
    ...
    hv_deepic2_result = run_hv_deepic2_dtlz7(
        args,
        deepic=deepic,
        plot=False,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train HV_DeepIC2 on 15D/20D/25D mixed-source problems and evaluate on DTLZ7."
    )
